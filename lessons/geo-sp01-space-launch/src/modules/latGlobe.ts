 ...
 * 06_资源引用.md），零远程请求；经纬网为贴球面的细线（不悬浮），与影像经纬度严格对齐；
 * 球缘一圈大气光晕由背面加法混合着色器生成。贴图为教学示意。
 */
import {
  AdditiveBlending, AmbientLight, BackSide, BufferAttribute, BufferGeometry,
  Clock, Color, ConeGeometry, DirectionalLight, DoubleSide, Group, Line,
  LineBasicMaterial, Mesh, MeshBasicMaterial, MeshPhongMaterial, PerspectiveCamera,
  Scene, ShaderMaterial, SphereGeometry, SRGBColorSpace, Texture, TextureLoader,
...
/**
 * 经纬网：贴球面的细线（随球体一起旋转，不是悬浮圆环）。
 * - 30° 网格（纬线 ±30°/±60° + 经线每 30°）：极淡主色，只作空间参照，不与教学内容抢视线；
 * - 五条教学纬线（赤道 / ±30° / ±60°）：按三档配色加亮，与扇形楔、右侧对比条一一对应；
 *   北半球是演示侧，故比同纬度南半球亮一档。
 * 半径自 R×1.001 递增到 R×1.004，各层不共面，避免 z-fighting。
 */
function buildGraticule(
  ...
  const add = (points: Vector3[], color: string, opacity: number): void => {
  ...
  for (const lat of [-60, -30, 30, 60]) add(circlePoints(lat, R * 1.002), primary, 0.16)
  for (let lon = -180; lon < 180; lon += 30) add(meridianPoints(lon, R * 1.001), primary, 0.12)

  const teaching: [number, string, number][] = [
    [0, primary, 0.95],
    [30, accent, 0.6],
    [60, accent, 0.5],
    [-30, accent, 0.34],
  ...
  for (const [lat, color, opacity] of teaching) add(circlePoints(lat, R * 1.004), color, opacity)
  return group
}
...
export function createLatGlobe(host: HTMLElement, points: LatitudePoint[]): LatGlobeHandle | null {
  ...
  const C_SPEC = cssVar('--c-text', '#e8eff8')
  ...
  const sun = new DirectionalLight(0xfff3e0, 1.0)
  ...
  earthGroup.rotation.y = -(EARTH_FACE_LON + 90) * DEG
  ...
  const earthMat = new MeshPhongMaterial({
    map: texture,
    // 低高光能量 + 近白镜面色：只让海面出现一点高光，读出"球"而不是"圆盘"
    shininess: 14,
